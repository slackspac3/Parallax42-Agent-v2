...
function fieldFromQuestion(question = '') {
  ...
  if (/(ai capabilities|private assistant|retrieval|document intelligence|policy q a|meeting summaries|internal employees|external customers|regulated|high risk workflows|compliance decisions|hr matters|human resources matters)/.test(text)) return 'ai_usage_scope';
...
function normalizeKnownGapField(value = '') {
  ...
  if (/ai usage|ai capabilities|internal employees|external customers|regulated workflows|high risk workflows|compliance decisions|hr matters|human resources matters/.test(text)) return 'ai_usage_scope';
...
function inferAiUsageScope(text = '', previousDraft = {}) {
  ...
  const hasScopeLanguage = /\b(internal|employee|employees|staff|external|customer|customers|client|clients|public|hr|human resources|compliance|regulated|high[- ]risk|workflow|use case|decision|matter)\b/i.test(clean);
  if (!aiQuestionContext && !hasScopeLanguage) return null;

  const scope = {};
  if (/\b(?:internal(?:ly)?|employee|employees|staff|workforce|company users?)\b/i.test(clean)) {
    scope.audience = /\b(?:only|solely|just)\b[^.]{0,35}\b(?:internal|employee|employees|staff)\b|\b(?:internal|employee|employees|staff)\b[^.]{0,35}\b(?:only|solely|just)\b/i.test(clean)
...
function mergeDraft(existing = {}, extracted = {}) {
  ...
  return {
    ...
    aiUsageScope: extracted.aiUsageScope || existing.aiUsageScope || null,
