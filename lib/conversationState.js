...
const INTEGRATION_PATTERNS = [
  ...
  ['Finance reporting', /\b(finance reporting|ledger|erp|oracle erp|erp export|payment approval|invoice)\b/i],
  ['CRM', /\b(crm|salesforce|dynamics crm)\b/i],
  ['Analytics platform', /\b(analytics|dashboard|bi|power bi|reporting|snowflake)\b/i],
...
const RISK_PATTERNS = [
  ...
  ['personal data', /\b(personal data|pii|employee data|employee records?|payroll|payroll data|salary data|compensation data|customer data|patient data|health data|medical data|sensitive data)\b/i],
  ...
  ['outsourced service', /\b(outsourc\w*|managed service|vendor|supplier|third party|third-party)\b/i],
  ['missing evidence', /\b(?:no|missing|without)\s+(?:final\s+|signed\s+|approved\s+)?(?:evidence|documents?|contract|agreement|dpa|soc\s*2|iso\s*27001|certificate|permit|classification|screening|runbook|approval|attestation|proof)\b|\b(?:evidence|documents?|contract|agreement|dpa|certificate|permit|classification|screening|runbook|approval|attestation|proof)\b[^.?!]{0,45}\b(?:missing|not\s+attached|unavailable)\b/i]
];
...
function hasNonAuthoritativeAttributionLanguage(value = '') {
  const text = cleanText(value).replace(/[’‘]/g, "'");
  return /^(?:scenario|quote[d]?|training|policy|document|reference|fixture|prompt|example|sample(?:\s+(?:input|data|text))?|hypothetical|illustration|demo|test(?:\s+(?:input|data|text))?)\s*:/i.test(text)
    || /^(?:i\s+(?:think|believe|guess|suspect|reckon|assume|suppose|expect|understand|hear(?:d)?)|we\s+(?:think|believe|guess|suspect|reckon|assume|suppose|expect|understand|hear(?:d)?)|as\s+(?:noted|stated|reported|described)\s+(?:by|in)\b|someone\s+mentioned\b|word\s+is\b)/i.test(text)
    || /\b(?:the\s+following|quoted\s+below|copied\s+from|taken\s+from|excerpted\s+from)\b/i.test(text)
    || /\b(?:the\s+)?(?:document|contract|attachment|clause|policy|memo|system|source|email|evidence|legal|report|audit|file|spreadsheet|database|record|analyst|vendor|supplier|someone)\s+(?:mentions?|notes?|lists?|says?|states?|reports?|claims?|asserts?|indicates?|suggests?|records?|believes?|expects?|confirms?|requires?|defines?|describes?|quotes?)\b/i.test(text)
    || /\b(?:i\s+guess|to\s+my\s+knowledge|according\s+to|per\s+[A-Za-z][\w.-]*|says?\s+[A-Za-z][\w.-]*|it\s+(?:seems|is\s+said|is\s+reported|is\s+believed|is\s+thought)|is\s+(?:thought|believed|purported|assumed|said|reported)\s+to|appears?\s+to|word\s+is|rumou?r\s+has\s+it)\b/i.test(text)
    || /\b(?:[A-Za-z][A-Za-z0-9&.'-]*(?:\s+[A-Za-z][A-Za-z0-9&.'-]*){0,4})\s+(?:says?|states?|notes?|reports?|claims?|asserts?|indicates?|suggests?|records?|lists?)\s+(?:that\s+)?\b/i.test(text);
}

function hasInterrogativeSentenceShape(value = '') {
  const text = cleanText(value);
  if (/^(?:who|what|which|where|when|why|how)\b/i.test(text)) return true;
  return /^(?:is|are|was|were|do|does|did|can|could|would|will|should|has|have|had)\s+(?!(?:be|been|not|never|missing|pending|unknown|unavailable|available|ready|final|confirmed|used|processed|hosted|deployed|integrated)\b)/i.test(text);
}

function hasNonAssertiveGapLanguage(value = '') {
  const text = cleanText(value).replace(/[’‘]/g, "'");
  return !text
    || /\?/.test(text)
    || hasInterrogativeSentenceShape(text)
    || hasNonAuthoritativeAttributionLanguage(text)
    || /\b(?:if|unless|whether|assume|assuming|suppose|supposing|maybe|may|might|could|perhaps|probably|possibly|potentially|hypothetical(?:ly)?|tentative(?:ly)?|provisional(?:ly)?|reportedly|apparently|allegedly|purportedly|unverified|disputed|for\s+(?:example|testing|reference)|subject\s+to\s+(?:confirmation|validation))\b/i.test(text);
}
...
function inferKnownGapAnswer(text = '', previousDraft = {}) {
  if (!isUnknownOrPending(text) || hasNonAssertiveGapLanguage(text)) return '';
...
function answeredFieldsFromExtracted(extracted = {}) {
  ...
  if (hasSubmittedEvidence(extracted)) fields.push('evidence');
  if (hasEvidenceSignal(extracted, /classification|end-use|import permit|sanctions/i)) fields.push('export_control_evidence');
  if (hasEvidenceSignal(extracted, /firmware|remote support|identity and access|mfa|session/i)) fields.push('remote_support_controls');
  if (hasEvidenceSignal(extracted, /sanctions/i)) fields.push('sanctions_screening');
...
function signalMatchIsNegated(clause = '', match = {}, label = '') {
  ...
  const scopedBefore = before.replace(/\bnot\s+only\b/gi, '');
  const immediateObjectNegation = /(?:\b(?:no|neither|nor|without|lacks?|lacking|excluded|instead\s+of|rather\s+than)\s+(?:any\s+)?|\bnon[- ]?)$/i.test(scopedBefore)
    || /\b(?:not|never|cannot|can'?t|does\s+not|doesn'?t|do\s+not|don'?t|will\s+not|won'?t)\s+(?:[A-Za-z-]+\s+){0,4}$/i.test(scopedBefore)
    || /\b(?:prohibited|blocked)\s+from\s+(?:[A-Za-z-]+\s+){0,4}$/i.test(scopedBefore)
    || /^(?:no|neither)\b[^,;]{0,65}\b(?:and|nor)\s+$/i.test(scopedBefore)
    || /\b(?:neither|nor)\b[^,;]{0,75}$/i.test(scopedBefore)
    || /\b(?:does\s+not|doesn'?t|do\s+not|don'?t|cannot|can'?t|never)\s+(?:use|integrate|process|connect|access|handle|store|host|apply)\b[^,;]{0,55}\band\s+$/i.test(scopedBefore);
  if (immediateObjectNegation) return true;
  const commonStatus = /^(?:[^.;?!]{0,22})\b(?:not\s+applicable|ruled\s+out|out\s+of\s+scope|excluded|denied|absent|removed|disabled|decommissioned)\b/i.test(after);
  if (commonStatus) return true;
  if (label === 'AI/model use') {
    return /^(?:[^.;?!]{0,18})(?:\b(?:does\s+not|doesn'?t)\s+apply\b|\b(?:(?:is|was)\s+not|isn'?t|wasn'?t|will\s+not|won'?t|cannot|can'?t)\s+(?:be\s+)?used\b|[- ]free\b)/i.test(after);
  }
  if (label === 'personal data') {
    return /^(?:[^.;?!]{0,22})\b(?:(?:is|are|was|were|will)\s+not|isn'?t|aren'?t|wasn'?t|weren'?t|won'?t|cannot|can'?t)\s+(?:be\s+)?(?:processed|used|stored|accessed|shared|transferred)\b|^(?:[^.;?!]{0,22})\b(?:prohibited|blocked)\b/i.test(after);
  }
  if (INTEGRATION_PATTERNS.some(([integrationLabel]) => integrationLabel === label)) {
    return /^(?:[^.;?!]{0,22})(?:\b(?:(?:is|are|was|were|will)\s+not|isn'?t|aren'?t|wasn'?t|weren'?t|won'?t|cannot|can'?t)\s+(?:be\s+)?(?:used|integrated|connected|enabled|configured)\b|\b(?:does\s+not|doesn'?t)\s+(?:integrate|connect|apply)\b)/i.test(after);
  }
  return false;
}

function caseSignalClauses(text = '') {
  const sentences = cleanText(text).replace(/[’‘]/g, "'").match(/[^.!?;]+[.!?;]*/g) || [];
  return sentences.flatMap((sentence) => {
    const contrastGroups = sentence.split(/\b(?:but|however|although|whereas|while|though|yet|except(?:\s+for)?)\b/i);
    return contrastGroups.flatMap((group) => {
      const clauses = group
        .split(/,/)
        .flatMap((clause) => clause.split(/\band\s+(?=[^,.;!?]{0,55}\b(?:is|are|was|were|will|uses?(?!\s+cases?\b)|used|integrates?|integrated|processes?|processed|connects?|connected|accesses?|accessed|handles?|handled|stores?|stored|hosts?|hosted|applies?|provided|ships?|shipped)\b)/i))
        .map(cleanText)
        .filter(Boolean);
      let inheritedNonAuthoritative = false;
      return clauses.map((clause) => {
        if (hasNonAssertiveCaseSignalLanguage(clause)) inheritedNonAuthoritative = true;
        return {
          text: clause,
          authoritative: !inheritedNonAuthoritative
        };
      });
    });
  });
...
function signalPatternStates(text = '', pattern, label = '') {
  const clauses = caseSignalClauses(text);
  let affirmative = false;
  let negated = false;
  for (const clauseState of clauses) {
    if (!clauseState.authoritative) continue;
    const clause = clauseState.text;
    const flags = pattern.flags.includes('g') ? pattern.flags : `${pattern.flags}g`;
    const matcher = new RegExp(pattern.source, flags);
    let match;
    while ((match = matcher.exec(clause)) !== null) {
      if (signalMatchIsNegated(clause, match, label)) negated = true;
...
function extractMentionedByPatterns(text, patterns) {
  const mentioned = [];
  for (const clauseState of caseSignalClauses(text)) {
    if (clauseState.authoritative) continue;
    const clause = clauseState.text;
    for (const [label, pattern] of patterns) {
      if (pattern.test(clause)) mentioned.push(label);
    }
  }
  return unique(mentioned);
}

function extractAssertedByPatterns(text, patterns) {
  return patterns
    .filter(([label, pattern]) => signalPatternStates(text, pattern, label).affirmative)
    .map(([label]) => label);
}

function extractNegatedByPatterns(text, patterns) {
  return patterns
    .filter(([label, pattern]) => signalPatternStates(text, pattern, label).negated)
    .map(([label]) => label);
}

function extractContradictedByPatterns(text, patterns) {
  return patterns
    .filter(([label, pattern]) => {
      const states = signalPatternStates(text, pattern, label);
      return states.affirmative && states.negated;
    })
    .map(([label]) => label);
}
...
function hasNonAssertiveCaseSignalLanguage(value = '') {
  const text = cleanText(value).replace(/[’‘]/g, "'");
  return !text
    || /\?/.test(text)
    || hasInterrogativeSentenceShape(text)
    || hasNonAuthoritativeAttributionLanguage(text)
    || /\b(?:if|unless|whether|either|or|assume|assuming|suppose|provided|maybe|may|might|could|perhaps|probably|possibly|potentially|tentative(?:ly)?|provisional(?:ly)?|pending\s+confirmation|awaiting\s+(?:confirmation|validation)|subject\s+to\s+(?:confirmation|validation)|according\s+to|reported(?:ly)?|apparently|supposedly|allegedly|unverified|rumou?r|heard|told|believes?|based\s+on|as\s+claimed|for\s+example|example|samples?|hypothetical(?:ly)?|illustrat(?:e|ed|ive|ion)|demo|reference\s+material|quoted\s+text|excerpt|training\s+text|test\s+(?:input|text|fixture)|placeholder|mock|fictional|counterexample|false\s+statement|untrusted\s+text)\b/i.test(text)
    || /\b(?:the\s+)?(?:document|contract|attachment|clause|policy|memo|system|source|email|evidence|legal)\s+(?:mentions?|notes?|lists?|says?|states?|reports?|claims?|indicates?|suggests?|records?|believes?)\b/i.test(text);
}
...
function hasNonAssertiveGeographyLanguage(value = '') {
  const text = cleanText(value).replace(/[’‘]/g, "'");
  return !text
    || /\?/.test(text)
    || hasInterrogativeSentenceShape(text)
    || hasNonAuthoritativeAttributionLanguage(text)
    || /\b(?:do\s+not|don'?t|not|never|no\s+longer|no\s+confirmation|without|except|other\s+than|if|unless|whether|assume|assuming|suppose|provided|maybe|may|might|could|perhaps|probably|possibly|potentially|uncertain|unclear|disputed|tentative(?:ly)?|provisional(?:ly)?|pending|awaiting|to\s+be\s+confirmed|subject\s+to\s+(?:confirmation|validation)|either|or|alternatively|according\s+to|reported(?:ly)?|apparently|supposedly|allegedly|unverified|rumou?r|heard|told|believes?|per\s+the|based\s+on|as\s+claimed|for\s+(?:example|testing|reference)|example|samples?|hypothetical(?:ly)?|illustrat(?:e|ed|ive|ion)|demo|reference\s+material|quoted\s+text|excerpt|training\s+text|test\s+(?:input|text|fixture)|placeholder|mock|fictional|counterexample|false\s+statement|untrusted\s+text)\b/i.test(text)
    || /\b(?:the\s+)?(?:document|contract|attachment|clause|policy|memo|system|source|email|evidence|legal)\s+(?:mentions?|notes?|lists?|says?|states?|reports?|claims?|indicates?|suggests?|records?|believes?)\b/i.test(text);
}

function geographyValueResidue(value = '') {
  let residue = cleanText(value);
  for (const [, pattern] of GEOGRAPHY_PATTERNS) {
    residue = residue.replace(new RegExp(pattern.source, 'gi'), ' ');
  }
  return residue
    .replace(/\b(?:also|add|include|including|plus|only|the|geography|jurisdiction|country|region|and|as\s+well)\b/gi, ' ')
    .replace(/[,+&./()-]/g, ' ')
    .replace(/\s+/g, '')
    .trim();
}

function inferGeography(text = '', previousDraft = {}) {
  if (hasNonAssertiveGeographyLanguage(text)) return '';
  const hits = unique(extractByPatterns(text, GEOGRAPHY_PATTERNS));
  if (!hits.length) return '';
  const clean = cleanText(text);
  const bareGeographyAnswer = !geographyValueResidue(clean);
  const explicitMatch = clean.match(/(?:^|[.;]\s*|\band\s+)(?:(?:the|its|our)\s+)?(?:geography|jurisdiction|country|region|regulatory\s+perimeter)\s*(?:is|=|:|includes?|covers?|applies\s+to)\s+([^.;!?]{1,160})/i)
    || clean.match(/^(?:change|update|replace|correct)\s+(?:the\s+)?(?:geography|jurisdiction|country|region|regulatory\s+perimeter)\s+(?:to|with)\s+([^.;!?]{1,160})/i);
  const explicitLabel = Boolean(explicitMatch?.[1] && !geographyValueResidue(explicitMatch[1]));
  const locationStatement = /\b(?:based|located|operat(?:e|es|ing)|deploy(?:ed|ing)?|host(?:ed|ing)?|used|applies?|ship(?:s|ping)?|deliver(?:s|ed|ing)?|supplier|vendor|company|service|workflow|case|request)\b[^.?!]{0,90}\b(?:in|to|for|across|within)\b/i.test(clean);
  const intakeInstruction = /^(?:please\s+)?(?:review|assess|check|analyse|analyze|evaluate|onboard|approve|procure|deploy)\b/i.test(clean);
  if (!bareGeographyAnswer && !explicitLabel && !locationStatement && !intakeInstruction) return '';
...
function inferContextualKnownGaps(text = '') {
  const clean = cleanText(text);
  if (hasNonAssertiveGapLanguage(clean)) return [];
  const gaps = [];
...
function fieldSatisfiedByExtracted(field = '', extracted = {}) {
  ...
  if (field === 'sanctions_screening') {
    return hasEvidenceSignal(extracted, /sanctions screening/i);
  }
  ...
  if (field === 'evidence') {
    return hasSubmittedEvidence(extracted);
  }
  if (field === 'export_control_evidence') {
    return hasEvidenceSignal(extracted, /classification|end-use|import permit|sanctions/i);
  }
  if (field === 'remote_support_controls') {
    return hasEvidenceSignal(extracted, /firmware|remote support|identity and access|mfa|session/i);
  }
  return true;
}
...
function hasSubmittedEvidence(draft = {}) {
  return Boolean(
    draft.documents?.some(isUsableEvidenceDocument)
    || draft.retrievalContext?.matches?.some(isUsableRetrievalEvidenceDocument)
    || draft.retrievalContext?.evidenceMatches?.some(isUsableRetrievalEvidenceDocument)
  );
}
...
function hasNonAssertiveOwnerLanguage(value = '') {
  const text = cleanText(value).replace(/[’‘]/g, "'");
  return !text
    || /\?/.test(text)
    || hasInterrogativeSentenceShape(text)
    || hasNonAuthoritativeAttributionLanguage(text)
    || /,\s*(?:right|correct)(?:[.?!]|$)/i.test(text)
    || /\b(?:for\s+example|example|samples?|hypothetical(?:ly)?|imagine|ignore|illustrat(?:e|ed|ive|ion)|demo|reference\s+material|quoted\s+text|excerpt|training\s+text|test\s+(?:input|text|fixture)|placeholder|mock|fictional|counterexample|false\s+statement|untrusted\s+text)\b/i.test(text)
    || /\b(?:the\s+)?(?:evidence|document|contract|system|clause|policy|memo)\s+(?:confirms?|records?|instructs?|requires?|infers?|inferred|reads?|mentions?|lists?|says?|states?|reports?|claims?)\b/i.test(text)
    || /\b(?:not|isn'?t|never|no\s+longer|ruled\s+out|cannot|can'?t|won'?t|except|other\s+than|hardly|maybe|might|perhaps|probably|possibly|likely|potentially|presumably|ostensibly|purportedly|definitely|assuming|suppose|provided|reportedly|apparently|supposedly|allegedly|unverified|appears?|seems?|according\s+to|subject\s+to\s+confirmation|in\s+the\s+event|as\s+long\s+as|either|or|alternatively|says?|states?|reports?|claims?|heard|told|sources?|indicates?|per|but)\b/i.test(text);
}

function hasNonAssertiveSupplierLanguage(value = '') {
  const text = cleanText(value).replace(/[’‘]/g, "'");
  return !text
    || /\?/.test(text)
    || hasInterrogativeSentenceShape(text)
    || hasNonAuthoritativeAttributionLanguage(text)
    || /\b(?:for\s+example|example|samples?|imagine|ignore|illustrat(?:e|ed|ive|ion)|demo|reference\s+material|quoted\s+text|excerpt|training\s+text|test\s+(?:input|text|fixture)|placeholder|mock|fictional|counterexample|false\s+statement|untrusted\s+text)\b/i.test(text)
    || /\b(?:the\s+)?(?:document|contract|clause|policy|memo|system)\s+(?:instructs?|requires?|reads?|mentions?|lists?|says?|states?|reports?|claims?)\b/i.test(text)
    || /\b(?:do\s+not|don'?t|not|never|no\s+longer|no\s+confirmation|away\s+from|unknown|unclear|unsure|uncertain|disputed|maybe|may|might|could|perhaps|probably|possibly|likely|potential|candidate|former|previous|rejected|backup|secondary|hypothetical|option|pending|awaiting|tentative|provisional(?:ly)?|draft|proposed|subject\s+to|based\s+on|as\s+claimed|testing|discussion\s+only|for\s+reference|demonstration|training|i\s+(?:think|believe)|reportedly|apparently|supposedly|allegedly|unverified|according\s+to|sources?|indicates?|says?|states?|reports?|claims?|heard|told|mentions?|lists?|considered|policy|memo|per|if|unless|whether|either|or|should\s+we|can\s+we|could\s+we)\b/i.test(text);
}

function normalizeExplicitOwnerCandidate(value = '', { allowNamedOrganization = false } = {}) {
  const candidate = cleanText(value)
    .replace(/^(?:(?:i|we)\s+(?:think|believe|expect|(?:can\s+)?confirm|understand|know|would\s+say)\s+(?:that\s+)?|(?:only|our)\s+)/i, '')
    .replace(/^(?:the)\s+/i, '')
    .trim();
  if (!candidate || candidate.split(/\s+/).length > 7) return '';
  if (/^(?:Legal\s+and\s+Privacy|Trade\s+Compliance\s+and\s+Export\s+Controls|Procurement\s+and\s+Third[- ]Party\s+Risk|HSE\s+and\s+Business\s+Continuity)$/i.test(candidate)) {
    return normalizeOwnerValue(candidate);
  }
  if (/[\/?;:]/.test(candidate)) return '';
  if (/\b(?:who|which|what|where|when|why|how|nobody|anybody|anyone|anything|other|none|unknown|pending|tbd|not|no|never|except|than|hardly|cannot|can'?t|won'?t|ruled|out|need|determine|figure|find|own|owns|owned|is|are|was|were|and|or|per|according|reportedly|apparently|supposedly|allegedly|potentially|presumably|ostensibly|purportedly|definitely|likely|alternatively|says?|states?|reports?|heard|suppose|provided|appears?|seems?|looks?)\b/i.test(candidate)) return '';
  const normalizedWords = candidate.toLowerCase().split(/\s+/);
  const functionalStarts = new Set([
    'hr', 'it', 'hse', 'legal', 'finance', 'procurement', 'security', 'compliance', 'risk', 'privacy',
    'technology', 'operations', 'platform', 'product', 'engineering', 'head', 'director', 'manager', 'lead',
    'group', 'trade', 'infrastructure', 'enterprise', 'data', 'digital', 'business', 'people', 'human',
    'payroll', 'customer', 'commercial', 'sales', 'marketing', 'support', 'third-party',
    'complianc', 'complaince', 'complicance', 'compliane', 'compliancee', 'compliace', 'procuremnt',
    'procurment', 'procurmentrisk', 'finace', 'financ', 'technolgy', 'techrisk', 'technologyrisk',
    'legalprivcy', 'legalprivacy', 'privcy'
  ]);
  const organizationalSuffixes = new Set(['team', 'unit', 'office', 'department', 'committee', 'council', 'function', 'group', 'factory']);
  const allowedOwnerWords = new Set([
    ...functionalStarts,
    ...organizationalSuffixes,
    'of', 'the', 'transformation', 'controls', 'assurance', 'governance', 'affairs', 'success',
    'architecture', 'delivery', 'services', 'systems', 'strategy', 'sourcing', 'vendor', 'export',
    'excellence', 'third', 'party'
  ]);
  const startsWithFunction = functionalStarts.has(normalizedWords[0]);
  const endsWithOrganization = organizationalSuffixes.has(normalizedWords.at(-1));
  if (!startsWithFunction && !(allowNamedOrganization && endsWithOrganization)) return '';
  if (startsWithFunction && normalizedWords.some((word) => !allowedOwnerWords.has(word))) return '';
  if (endsWithOrganization) {
    const rawWords = candidate.split(/\s+/);
    const invalidRawWord = rawWords.some((word) => {
      const normalized = word.toLowerCase();
      if (allowedOwnerWords.has(normalized)) return false;
      return !/^[A-Z][A-Za-z0-9&.'-]*$/.test(word);
    });
    if (invalidRawWord) return '';
  }
  return normalizeOwnerValue(candidate);
}

function looksLikeLeadingOwnerAssertion(value = '') {
  const text = cleanText(value);
  return /^(?:the\s+)?(?:business\s+unit|workflow\s+owner|case\s+owner|accountable\s+(?:team|owner|business\s+unit)|responsible\s+(?:team|owner|unit)|owner)\b/i.test(text)
    || /^(?:the\s+)?(?:responsibility|accountability)\b/i.test(text)
    || /^(?:(?:this|the)\s+)?(?:case|review|workflow|assessment|request)\s+(?:is\s+)?owned\s+by\b/i.test(text)
    || /^(?:the\s+)?(?:head|director|vp|manager|lead)\s+of\s+.+?\s+(?:is|will be)\s+(?:the\s+)?(?:responsible|accountable)\b/i.test(text)
    || /^(?:(?:i|we)\s+(?:(?:can\s+)?confirm|know|understand)\s+(?:that\s+)?)?(?:the\s+)?[A-Za-z][A-Za-z0-9&/ '-]{1,80}?\s+(?:owns|will own|is responsible for|is accountable for)\s+(?:it|this|the\s+(?:case|review|workflow|assessment|request))\b/i.test(text);
}

function isDirectIntakePrefix(value = '') {
  const text = cleanText(value);
  return /^(?:review|assess|focus\s+on|prioriti[sz]e)\s+(?:all\s+(?:listed\s+)?(?:risks?|areas?)|all\s+of\s+(?:the\s+)?(?:risks?|areas?))$/i.test(text);
}

function isTrustedOwnerTail(value = '') {
  const text = cleanText(value);
  if (containsOwnershipSemantics(text)) return false;
  return /^(?:(?:the|its?|our)\s+)?(?:geography|jurisdiction|country|regulatory\s+perimeter)\b/i.test(text)
    || /^(?:(?:the|our)\s+)?shipment\s+(?:includes?|contains?|ships?|will\s+(?:include|contain|ship))\b/i.test(text)
    || /^(?:final\s+)?(?:end[- ]use|end[- ]user|export\s+origin|origin\s+jurisdiction|exporting\s+jurisdiction|sanctions\s+screening|import\s+destination)\s*(?:is|=|:|will\s+be)\b/i.test(text)
    || /^(?:(?:the|our)\s+)?(?:supplier|vendor|platform|service)\s+(?:(?:is|will\s+be)\s+(?:based|located|operating|processing|using|connected|hosted)|will\s+(?:provide|process|operate|ship|use|integrate|connect|handle|store|host)|provides?|processes?|operates?|ships?|uses?|integrates?|connects?|handles?|stores?|hosts?)\b/i.test(text)
    || /^(?:attached|available)\s+(?:export\s+classification|end[- ]use\s+certificate|import\s+permit|sanctions\s+screening|dpa|data\s+processing\s+agreement|soc\s*2|iso\s*27001|bcp|continuity\s+plan|support\s+runbook)\b/i.test(text)
    || /^it\s+(?:uses?|processes?|connects?|integrates?|ships?|provides?)\b/i.test(cleanText(value));
}

function containsOwnershipSemantics(value = '') {
  return /\b(?:owns?|owner|owners|owned\s+by|ownership|accountable|accountability|responsible|responsibility|decision\s+owner|final\s+authority|authority|belongs?\s+to|sits?\s+with|retains?\s+accountability|has\s+final\s+authority)\b/i.test(cleanText(value));
}

function authoritativeOwnerAssertionSegment(value = '') {
  const segments = cleanText(value)
    .split(/[.!?;](?:\s+|$)/)
    .map(cleanText)
    .filter(Boolean);
  const claimIndex = segments.findIndex(looksLikeLeadingOwnerAssertion);
  if (claimIndex < 0) return '';
  if (segments.slice(0, claimIndex).some((segment) => !isDirectIntakePrefix(segment))) return '';
  if (segments.slice(claimIndex + 1).some((segment) => looksLikeLeadingOwnerAssertion(segment) || containsOwnershipSemantics(segment))) return '';
  if (segments.slice(claimIndex + 1).some((segment) => !isTrustedOwnerTail(segment))) return '';
  return segments[claimIndex];
}

function explicitBusinessOwnerFromText(value = '') {
  const originalText = cleanText(value);
  if (hasNonAssertiveOwnerLanguage(originalText)) return '';
  // Only a leading assertion or one preceded by a direct intake command is
  // authoritative. Arbitrary quoted/reference sentences cannot create trust.
  const text = authoritativeOwnerAssertionSegment(originalText);
  if (!text) return '';
  if (text.split(',').slice(1).some((tail) => {
    const cleanTail = cleanText(tail);
    return containsOwnershipSemantics(cleanTail)
      || !/^(?:and\s+)?(?:the\s+)?(?:geography|jurisdiction|country|regulatory\s+perimeter)\b/i.test(cleanTail);
  })) return '';
  if (/\b(?:if|unless|whether|maybe|might|possible|possibly|potential|probably|assuming|suppose|provided|reported|reportedly|apparent|apparently|presumed|supposed|supposedly|alleged|allegedly|purported|unconfirmed|candidate|proposed|former|previous|draft|backup|secondary|unverified|according\s+to|subject\s+to\s+confirmation|could\s+be|would\s+be|as\s+long\s+as)\b/i.test(text)) return '';
  if (/\b(?:i|we)\s+(?:think|believe|expect|guess|suppose)\b/i.test(text)) return '';
  if (/\b(?:says?|heard|per|but|sources?|indicates?|told)\b/i.test(text)) return '';
  if (/\b(?:owner|owned\s+by|accountable|responsible)\b[^.?!]{0,80}\b(?:or|\/|per)\b/i.test(text)) return '';
  const containsKnownCompoundOwner = /\b(?:Legal\s+and\s+Privacy|Trade\s+Compliance\s+and\s+Export\s+Controls|Procurement\s+and\s+Third[- ]Party\s+Risk|HSE\s+and\s+Business\s+Continuity)\b/i.test(text);
  if (
    !containsKnownCompoundOwner
    && /\b(?:owner|owned\s+by|accountable|responsible)\b[^.?!]{0,80}\band\b(?!\s+(?:the\s+)?(?:geography|jurisdiction|country|regulatory\s+perimeter)\b)/i.test(text)
  ) return '';
  if (/\b(?:does\s+not|doesn'?t|do\s+not|don'?t|never|nobody|no\s+one|no\s+team)\b/i.test(text)) return '';
  const ownerClaimEnd = '(?=$|[.!;](?:\\s|$)|,\\s+(?:and\\s+)?(?:the\\s+)?(?:geography|jurisdiction|country|regulatory\\s+perimeter)\\b|\\s+and\\s+(?:the\\s+)?(?:geography|jurisdiction|country|regulatory\\s+perimeter)\\b)';
  const patterns = [
    /^(?:the\s+)?(?:business\s+unit|workflow\s+owner|case\s+owner|accountable\s+(?:team|owner|business\s+unit)|responsible\s+(?:team|owner|unit)|owner)\s*(?:is|:|=|will be|should be)?\s+(?:the\s+)?([A-Za-z][A-Za-z0-9&/ '-]{1,80}?)(?=\.|,|;|\band\s+(?:the\s+)?(?:geography|jurisdiction|country|regulatory\s+perimeter)\b|$)/i,
    /^(?:the\s+)?(?:responsibility|accountability)\s+(?:sits with|belongs to|is with)\s+(?:the\s+)?([A-Za-z][A-Za-z0-9&/ '-]{1,80}?)(?=\.|,|;|\band\s+(?:the\s+)?(?:geography|jurisdiction|country|regulatory\s+perimeter)\b|$)/i,
    /^(?:the\s+)?((?:head|director|vp|manager|lead)\s+of\s+(?:it|[A-Z]{2,}|[A-Za-z][A-Za-z&/.-]+(?:\s+[A-Za-z&/.-]+){0,3}))\s+(?:is|will be)\s+(?:the\s+)?(?:responsible|accountable)(?=\.|,|;|\s+for\s+(?:this|the)\s+(?:case|review|workflow|assessment|request)|$)/i,
    new RegExp(`^(?:the\\s+)?([A-Za-z][A-Za-z0-9&/ '-]{1,80}?)\\s+(?:owns|will own|is responsible for|is accountable for)\\s+(?:it|this(?:\\s+(?:case|review|workflow|assessment|request))?|(?:the\\s+)?(?:case|review|workflow|assessment|request))(?:\\s+now)?${ownerClaimEnd}`, 'i'),
    /^(?:(?:this|the)\s+)?(?:case|review|workflow|assessment|request)\s+(?:is\s+)?owned\s+by\s+(?:the\s+)?([A-Za-z][A-Za-z0-9&/ '-]{1,80}?)(?=\.|,|;|\band\s+(?:the\s+)?(?:geography|jurisdiction|country|regulatory\s+perimeter)\b|$)/i
  ];
  for (const pattern of patterns) {
    const match = text.match(pattern);
    if (match?.[1]) {
      const owner = normalizeExplicitOwnerCandidate(match[1], { allowNamedOrganization: true });
...
function inferBusinessOwnerAnswer(text = '', previousDraft = {}) {
  ...
  if (
    hasNonAssertiveOwnerLanguage(text)
    || isUnknownOrPending(text)
    || /\?$/.test(cleanText(text))
    || /^(?:not\b|maybe|possibly|probably|unverified|subject\s+to\s+confirmation|i\s+(?:think|believe|guess)|we\s+(?:think|believe|guess)|sources?\s+(?:say|indicate)|we\s+were\s+told)\b/i.test(cleanText(text))
    || /\b(?:is\s+not|does\s+not|doesn'?t|not\s+sure|subject\s+to\s+confirmation)\b/i.test(cleanText(text))
  ) {
    return '';
  }
  const activeField = activeQuestionFieldForDraft(previousDraft);
  ...
  const explicitOwner = explicitBusinessOwnerFromText(text);
  if (explicitOwner) return explicitOwner;
  if (
    likelyAnsweringOwner
    && (!activeField || activeField === 'business_owner')
    && text.length <= 90
    && !/\?$/.test(text)
    && (likelyAnsweringOwner || !inferFirstByPatterns(text, GEOGRAPHY_PATTERNS))
    && shortOwnerPattern.test(text)
  ) {
    return normalizeExplicitOwnerCandidate(text);
  }
  return '';
}
...
function inferDataCategories(text = '', previousDraft = {}) {
  ...
  if (/\b(sensitive personal|health|patient|biometric|religion|political|special categor(?:y|ies)|criminal|medical)\b/i.test(clean)) {
...
function hasEvidenceSignal(draft = {}, pattern) {
  const usableDocuments = (draft.documents || []).filter(isUsableEvidenceDocument);
  const documentSignals = usableDocuments.flatMap((document) => document.signals || []);
  const retrievalMatches = [
    ...(draft.retrievalContext?.matches || []),
    ...(draft.retrievalContext?.evidenceMatches || [])
  ].filter(isUsableRetrievalEvidenceDocument);
  const retrievalSignals = retrievalMatches.flatMap((match) => match.signals || match.metadata?.signals || []);
  const attachedSignals = usableDocuments.length || retrievalMatches.length ? draft.evidenceSignals || [] : [];
  return [...documentSignals, ...retrievalSignals, ...attachedSignals].some((signal) => pattern.test(signal));
}
...
function isGenericSupplierCandidate(value = '') {
  const candidate = cleanText(value);
  if (!candidate) return true;
  if (/\b(?:inc\.?|ltd\.?|llc|corp(?:oration)?|company)\b/i.test(candidate)) return false;
  return /^(?:all|everything|full(?:[- ]scope)?\s+review|comprehensive\s+review|privacy|security|cybersecurity(?:\s+posture)?|compliance|risk|resilience|controls?|posture|identity\s+lifecycle|data\s+(?:processing|residency)|customer\s+data|audit\s+rights?|liability|termination|commercial|legal|access|cross[- ]border|retention|deletion|subprocessors?|service\s+levels?|tool\s+performance|operational(?:\s+(?:risk|resilience))?|business\s+continuity|model\s+governance|regulatory(?:\s+exposure)?|governance(?:\s+gaps?)?|sanctions(?:\s+exposure)?|financial(?:\s+exposure)?|group\s+risk|third[- ]party(?:\s+(?:risk|concentration|provider|service))?|vendor\s+risk|incident\s+response(?:\s+maturity)?|cloud(?:\s+exit(?:\s+readiness)?)?|hosting|payment|implementation|hr|technology\s+solutions|payroll(?!\s+outsourcing\b)|due\s+diligence|onboarding|export\s+control|contract|agreement|document|report|evidence|clauses?|terms?|managed|outsourc\w*|saas|software|platform|tool|service|application|product|provider|fraud\s+analytics)\b/i.test(candidate)
    || /^(?:the\s+)?(?:case|review|request|workflow)$/i.test(candidate);
}

function finalizeExplicitSupplierCandidate(remainder = '', candidate = '', consumedLength = 0) {
  const cleanCandidate = cleanText(candidate).replace(/[.?!,;:]+$/g, '');
  if (!cleanCandidate) return '';
  const tail = cleanText(remainder.slice(consumedLength));
  if (!tail || /^[.?!;]+$/.test(tail)) return cleanCandidate;
  if (/^(?:(?:will|shall|would)\s+(?:provide|handle|support|process|integrate|connect|ship|operate|host|store|use)|(?:has|have)\s+(?:operations?|offices?|employees?|customers?|contracts?|systems?)|provides?|handles?|supports?|processes?|integrates?|connects?|ships?|operates?|headquartered|based|located)\b/i.test(tail)) return cleanCandidate;
  if (/^in\b/i.test(tail) && extractByPatterns(tail, GEOGRAPHY_PATTERNS).length) return cleanCandidate;
  if (/^(?:is|are|was|were)\s+(?:based|located|headquartered|operating|providing|responsible|the\s+(?:supplier|vendor|provider|platform|tool|service))\b/i.test(tail)) return cleanCandidate;
  return '';
}

function explicitNamedSupplierFromText(value = '') {
  const text = cleanText(value);
  if (/[.!?;]\s+\S/.test(text)) return '';
  if (text.split(',').slice(1).some((tail) => !/^(?:and\s+)?(?:the\s+)?(?:geography|jurisdiction|country|business\s+unit|owner)\b/i.test(cleanText(tail)))) return '';
  const prefix = /^(?:the\s+)?(?:supplier|vendor|platform|tool|service)\s+(?:is|=|:|named|called)\s+/i.exec(text);
  if (!prefix) return '';
  const remainder = text.slice(prefix.index + prefix[0].length).trim().replace(/^now\s+(?:the\s+)?/i, '');
  const quoted = remainder.match(/^["']([^"']{1,80})["']/);
  if (quoted?.[1]) return finalizeExplicitSupplierCandidate(remainder, quoted[1], quoted[0].length);
  if (/^[a-z]/.test(remainder)) {
    const lowerCorporateName = remainder.match(/^([a-z0-9&.'-]+(?:\s+(?!(?:will|would|shall|is|are|was|were|has|have|in|from|at|for|with|that|headquartered|based|located)\b)[a-z0-9&.'-]+){0,4}\s+(?:inc\.?|ltd\.?|llc|corp(?:oration)?|company|group|systems?|technologies|solutions|services|holdings))\b/i);
    if (lowerCorporateName?.[1]) return finalizeExplicitSupplierCandidate(remainder, lowerCorporateName[1], lowerCorporateName[0].length);
  }
  const properName = remainder.match(/^([A-Z0-9][A-Za-z0-9&.'-]*(?:\s+(?:(?:[A-Z0-9][A-Za-z0-9&.'-]*)|&|and|of|the)){0,7})/);
  if (properName?.[1]) return finalizeExplicitSupplierCandidate(remainder, properName[1], properName[0].length);
  const firstToken = remainder.match(/^([A-Za-z0-9][A-Za-z0-9&.'-]*)/);
  return firstToken?.[1] ? finalizeExplicitSupplierCandidate(remainder, firstToken[1], firstToken[0].length) : '';
}

function hasExplicitSupplierIdentityClaim(value = '', candidate = '') {
  const text = cleanText(value);
  const namedCandidate = cleanText(candidate);
  if (!namedCandidate || isGenericSupplierCandidate(namedCandidate)) return false;
  if (hasNonAssertiveSupplierLanguage(text) || /[.!?;]\s+\S/.test(text)) return false;
  const explicitNamed = explicitNamedSupplierFromText(text);
  if (explicitNamed && explicitNamed.toLowerCase() === namedCandidate.toLowerCase()) return true;
  const escaped = namedCandidate.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  const candidateEnd = '(?=$|[.;!?]|,\\s+(?:and\\s+)?(?:the\\s+)?(?:geography|jurisdiction|country|business\\s+unit|owner)\\b|\\s+and\\s+(?:the\\s+)?(?:geography|jurisdiction|country|business\\s+unit|owner)\\b)';
  const directIdentity = new RegExp(`^(?:the\\s+)?(?:supplier|vendor|provider|platform|tool|service)\\s+(?:is|=|:|named|called)\\s+(?:now\\s+)?(?:the\\s+)?${escaped}${candidateEnd}`, 'i').test(text);
  if (directIdentity) return true;
  const hasChangeIntent = /\b(?:change|replace|correct|update)\b/i.test(text);
  if (hasChangeIntent) {
    return new RegExp(`^(?:please\\s+)?(?:change|replace|correct|update)\\s+(?:the\\s+)?(?:supplier|vendor|provider|platform|tool|service)(?:\\s+from\\s+[^.?!;]{1,80})?\\s+(?:to|with|as)\\s+(?:the\\s+)?${escaped}${candidateEnd}`, 'i').test(text);
  }
  return false;
}

function inferSupplierName(text = '', previousDraft = {}) {
  if (hasNonAssertiveSupplierLanguage(text)) return '';
  const explicitNamed = explicitNamedSupplierFromText(text);
  if (explicitNamed && !isGenericSupplierCandidate(explicitNamed)) {
    const establishedSupplier = cleanText(previousDraft.supplierName);
    if (
      !establishedSupplier
      || /^conversation-supplied case$/i.test(establishedSupplier)
      || establishedSupplier.toLowerCase() === explicitNamed.toLowerCase()
      || hasExplicitSupplierIdentityClaim(text, explicitNamed)
    ) {
      return explicitNamed;
    }
  }
  return '';
}
...
function extractCaseFields(message = '', previousDraft = {}) {
  const text = cleanText(message);
  const authoritativeSignalClauses = caseSignalClauses(text)
    .filter((clause) => clause.authoritative)
    .map((clause) => clause.text);
  const authoritativeSignalText = authoritativeSignalClauses.join('. ');
  const intakeAssessment = inferRequestProfileFromText(text);
  const updateMode = inferUpdateMode(text);
  const supplierName = inferSupplierName(text, previousDraft);
  const activeField = activeQuestionFieldForDraft(previousDraft);
  const geography = activeField === 'export_origin_jurisdiction' ? '' : inferGeography(text, previousDraft);
  const exportOriginJurisdiction = inferExportOriginJurisdiction(authoritativeSignalText, previousDraft);
  const exportEndUse = inferExportEndUse(authoritativeSignalText, previousDraft);
  ...
  const aiUsageScope = inferAiUsageScope(authoritativeSignalText, previousDraft);
  const reviewFocus = inferReviewFocus(authoritativeSignalText, previousDraft);
  const dataCategories = inferDataCategories(authoritativeSignalText, previousDraft);
  const integrations = extractAssertedByPatterns(text, INTEGRATION_PATTERNS);
  const conversationalEvidenceSignals = extractEvidenceSignals(text);
  const evidenceSignals = [];
  const sanctionsSensitiveGeographies = sanctionsSensitiveGeographiesFromText(authoritativeSignalText);
  let riskSignals = extractAssertedByPatterns(text, RISK_PATTERNS);
  ...
  const hasEvidenceLanguage = Boolean(
    conversationalEvidenceSignals.length > 0 && /\b(attached|available|provided|pasted|uploaded|we have|i have|evidence)\b/i.test(text)
  ...
  const documents = hasEvidenceLanguage
    ? [normaliseEvidenceDocument({
      ...
      title: 'Conversational evidence reference (not submitted)',
      sourceType: 'chat_message',
      extractionStatus: 'nlp_extracted',
      summary: text,
      excerpt: text.length > 260 ? `${text.slice(0, 260).trim()}...` : text,
      signals: unique(conversationalEvidenceSignals)
    })]
    : [];
  const evidenceAssertion = cleanText(documents[0]?.assertionState);
  ...
  return {
    ...
    exportEndUse,
    integrations,
    mentionedIntegrations: extractMentionedByPatterns(text, INTEGRATION_PATTERNS),
    negatedIntegrations: unique(extractNegatedByPatterns(text, INTEGRATION_PATTERNS)),
    contradictedIntegrations: unique(extractContradictedByPatterns(text, INTEGRATION_PATTERNS)),
    documents,
    evidenceSignals,
    requestedEvidenceSignals: evidenceAssertion === 'requested' ? unique(conversationalEvidenceSignals) : [],
    mentionedEvidenceSignals: evidenceAssertion !== 'requested' ? unique(conversationalEvidenceSignals) : [],
    riskSignals,
    mentionedRiskSignals: extractMentionedByPatterns(text, RISK_PATTERNS),
    negatedRiskSignals: unique(extractNegatedByPatterns(text, RISK_PATTERNS)),
    contradictedRiskSignals: unique(extractContradictedByPatterns(text, RISK_PATTERNS)),
    sanctionsSensitiveGeographies,
    knownGaps: unique(authoritativeSignalClauses.flatMap((clause) => {
      const knownGap = inferKnownGapAnswer(clause, previousDraft);
      return [...(knownGap ? [knownGap] : []), ...inferContextualKnownGaps(clause)];
    })),
    reviewFocus,
    aiUsageScope,
    dataCategories,
...
function mergeDraft(existing = {}, extracted = {}) {
  ...
  ].slice(-18).map(normaliseEvidenceDocument);
  ...
  return {
    ...
    mentionedIntegrations: unique([...(existing.mentionedIntegrations || []), ...(extracted.mentionedIntegrations || [])]).slice(0, 18),
    negatedIntegrations: unique([...(existing.negatedIntegrations || []), ...(extracted.negatedIntegrations || [])]).slice(0, 18),
    contradictedIntegrations: unique([...(existing.contradictedIntegrations || []), ...(extracted.contradictedIntegrations || [])]).slice(0, 18),
    documents,
    evidenceSignals: unique([...(existing.evidenceSignals || []), ...(extracted.evidenceSignals || [])]),
    requestedEvidenceSignals: unique([...(existing.requestedEvidenceSignals || []), ...(extracted.requestedEvidenceSignals || [])]).slice(0, 18),
    mentionedEvidenceSignals: unique([...(existing.mentionedEvidenceSignals || []), ...(extracted.mentionedEvidenceSignals || [])]).slice(0, 18),
    riskSignals: unique([...(existing.riskSignals || []), ...(extracted.riskSignals || [])]),
    mentionedRiskSignals: unique([...(existing.mentionedRiskSignals || []), ...(extracted.mentionedRiskSignals || [])]).slice(0, 18),
    negatedRiskSignals: unique([...(existing.negatedRiskSignals || []), ...(extracted.negatedRiskSignals || [])]).slice(0, 18),
    contradictedRiskSignals: unique([...(existing.contradictedRiskSignals || []), ...(extracted.contradictedRiskSignals || [])]).slice(0, 18),
...
function missingFields(draft = {}) {
  ...
  if (
    ...
    && !hasEvidenceSignal(draft, /export classification|end-use certificate|import permit/i)
  ) {
...
function runReadinessForDraft(draft = {}, missing = []) {
  ...
  const rawScore = Math.round(
    ...
    + Math.min(24, (draft.documents || []).filter(isUsableEvidenceDocument).length * 4)
...
function casePayloadFromDraft(draft = {}) {
  return {
    ...
    mentionedIntegrations: draft.mentionedIntegrations || [],
    negatedIntegrations: draft.negatedIntegrations || [],
    contradictedIntegrations: draft.contradictedIntegrations || [],
    riskSignals: draft.riskSignals || [],
    evidenceSignals: draft.evidenceSignals || [],
    requestedEvidenceSignals: draft.requestedEvidenceSignals || [],
    mentionedEvidenceSignals: draft.mentionedEvidenceSignals || [],
    mentionedRiskSignals: draft.mentionedRiskSignals || [],
    negatedRiskSignals: draft.negatedRiskSignals || [],
    contradictedRiskSignals: draft.contradictedRiskSignals || [],
...
module.exports = {
  ...
  hasExplicitSupplierIdentityClaim,
  hasSubmittedEvidence,
  inferRequestProfileFromText,
  hasNonAssertiveCaseSignalLanguage,
  isGenericSupplierCandidate,
