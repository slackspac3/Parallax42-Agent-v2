...
function reviewerDecisionTarget(value = '') {
  ...
  if (/^(?:approve(?:d)?|accept(?:ed)?)$/.test(decision)) {
...
async function handleCaseApproval({ req, body = {} } = {}) {
  ...
  // The server-owned readiness contract is the sole approval gate. Do not let a
  // stale, advisory, or contradictory decision field override it.
  const approvalEligible = current.lastRun?.decisionReadiness?.approvalEligible === true;
  if (target === CASE_STATES.APPROVED && !approvalEligible) {
    return {
      status: 409,
      body: {
        ok: false,
        error: 'case_not_approval_eligible',
        detail: 'The council did not mark this case approval-eligible. Resolve every condition, record remediation, and rerun the council before approval.'
      }
    };
  }
