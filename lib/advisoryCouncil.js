...
function summarizeRunForPrompt(run = {}) {
  ...
  return JSON.stringify({
    ...
    retrievalContext: {
      evidenceMatches: (retrieval.evidenceMatches || retrieval.matches || []).slice(0, 8).map((match) => ({
        evidenceId: match.evidenceId,
        title: match.title,
        score: match.score,
        snippet: cleanText(match.snippet || match.text).slice(0, 500)
      })),
      similarCases: (retrieval.similarCases || []).slice(0, 6),
      learningSuggestions: retrieval.learningSuggestions || null,
      missingEvidenceSignals: retrieval.missingEvidenceSignals || []
    },
    gaps: run.gaps,
    ...
    messages: [
      {
        ...
        content: [
          ...
          'You must not approve the case, override deterministic controls, or invent evidence.',
          'Use only supplied deterministic run output, evidence snippets, similar cases, and reviewer feedback.',
          ...
          'Set advisoryOnly to true.'
        ].join(' ')
      },
      {
        role: 'user',
        content: `Provide advisory-only specialist review for this deterministic council run.\n\n${summarizeRunForPrompt(run)}`
      }
    ]
  });
...
async function runGatewayAdvisoryCouncil(run = {}) {
  const specialists = [];
  for (const specialistConfig of SPECIALISTS) {
    try {
      specialists.push(await runSpecialistAdvisory(run, specialistConfig));
    } catch (error) {
      specialists.push({
