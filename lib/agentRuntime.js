...
async function runRemoteCrewAIFlow(input, { live = false, llm = false } = {}) {
  ...
  if (!token) {
    throw new Error('A CrewAI service token is required for remote requests; configure P42_CREWAI_SERVICE_TOKEN.');
  }
  const configuredTimeout = Number(process.env.P42_CREWAI_SERVICE_TIMEOUT_MS || 180000);
  const timeoutMs = Number.isFinite(configuredTimeout) && configuredTimeout > 0
    ? Math.max(1000, Math.min(300000, Math.round(configuredTimeout)))
    : 180000;
...
async function attachGatewayAdvisoryIfEnabled(result, options = {}) {
  ...
  const hasCrewAIOutput = Boolean(result?.orchestration?.crewaiOutput || result?.runtime?.llmOutput);
  if (!result?.ok || hasCrewAIOutput || !wantsLiveLlm || !llm.enabled || !llm.apiKeyConfigured || !isFeatureEnabled('liveAdvisorySpecialists')) {
    return result;
  }
  try {
