...
function auditStoreHealth() {
  ...
  const provider = process.env.P42_AUDIT_STORE_PROVIDER || (vercelEphemeral ? 'local_tmp' : 'local_file');
  const durable = !vercelEphemeral && provider !== 'local_tmp';
  return {
    ...
    durable,
    durableRequired,
    enterpriseReady: durable && provider !== 'local_file' && provider !== 'local_tmp',
    durabilityNote: vercelEphemeral
      ? 'Vercel runtime uses /tmp unless AGENT_AUDIT_DIR is backed by durable storage.'
      : provider === 'local_file'
        ? 'Audit file is written to the configured filesystem path. Use managed append-only storage for multi-instance production.'
        : 'Audit events are configured for durable managed storage.'
  };
}
