...
function normaliseInput(input = {}) {
  const suppliedEvidenceMatches = Array.isArray(input.retrievalContext?.evidenceMatches)
    ? input.retrievalContext.evidenceMatches
    : [];
  const rawMatches = suppliedEvidenceMatches.length
    ? suppliedEvidenceMatches
    : Array.isArray(input.retrievalContext?.matches) ? input.retrievalContext.matches : [];
  const normalisedMatches = rawMatches.slice(0, 12).map(normaliseRetrievalEvidenceDocument);
  const usableMatches = normalisedMatches.filter(isUsableEvidenceDocument);
  const retrievalContext = input.retrievalContext && typeof input.retrievalContext === 'object'
    ? {
      ...
      candidateMatchCount: normalisedMatches.length,
      matchCount: usableMatches.length,
      matches: normalisedMatches,
      evidenceMatches: usableMatches,
  ...
  return {
    ...
    integrations: Array.isArray(input.integrations)
      ...
      : [],
    documents: Array.isArray(input.documents) ? input.documents.slice(0, 18).map(normaliseEvidenceDocument) : [],
    retrievalContext
  };
}

function hasUsableEvidence(caseInput = {}) {
  return caseInput.documents.some(isUsableEvidenceDocument)
    || usableRetrievalMatches(caseInput).length > 0;
}

function usableRetrievalMatches(caseInput = {}) {
  const matches = Array.isArray(caseInput.retrievalContext?.matches)
    ? caseInput.retrievalContext.matches
    : Array.isArray(caseInput.retrievalContext?.evidenceMatches) ? caseInput.retrievalContext.evidenceMatches : [];
  return matches
    .filter(isUsableRetrievalEvidenceDocument)
    .map(normaliseRetrievalEvidenceDocument);
}
...
function hasPositiveRequirementEvidence(caseInput = {}, code = '') {
  const requirement = GAP_REQUIREMENTS[code];
  if (!requirement?.include) return false;
  const documents = (Array.isArray(caseInput.documents) ? caseInput.documents : []).filter(isUsableEvidenceDocument);
...
function buildGaps(caseInput, domains) {
  ...
  for (const domain of domains) {
    if (domain.contradiction) {
      gaps.push({
        code: `applicability_contradiction:${domain.id}`,
        severity: 'high',
        gap: `${domain.label} has contradictory applicability assertions.`,
        action: `Reconcile the conflicting ${domain.label.toLowerCase()} statements against the authoritative source before approval.`,
        assertions: domain.applicabilityAssertions
      });
    }
    if (domain.status === 'needs_confirmation' && !domain.contradiction) {
      gaps.push({
...
function hasPositiveDocumentEvidence(documents = [], includePattern, negativePattern) {
  return documents.filter(isUsableEvidenceDocument).some((doc) => {
...
function buildDecision(gaps, evidenceQuality = {}) {
  ...
  const hasApplicabilityContradiction = gaps.some((gap) => cleanText(gap.code).startsWith('applicability_contradiction:'));
  let decision;
  if (hasApplicabilityContradiction || highCount >= 2 || (highCount >= 1 && mediumCount >= 3)) {
    decision = {
      status: 'not_ready',
      recommendation: 'Do not approve yet',
      readinessScore: 0.32,
      rationale: hasApplicabilityContradiction
        ? 'Conflicting applicability assertions must be reconciled against an authoritative source.'
        : 'Multiple high-severity evidence or applicability gaps remain open.'
    };
  } else if (highCount || mediumCount) {
    decision = {
      status: 'conditionally_ready',
      recommendation: 'Continue review with named controls',
  ...
  } else if (evidenceQuality.status === 'missing' || evidenceQuality.status === 'weak') {
    decision = {
      status: 'conditionally_ready',
      recommendation: 'Continue review pending usable evidence',
      readinessScore: 0.52,
  ...
  return {
    ...decision,
    approvalEligible: decision.status === 'ready',
    humanApprovalRequired: true,
    blockingGaps: gaps.length,
    highSeverityGaps: highCount,
    mediumSeverityGaps: mediumCount,
...
function buildCitations(caseInput) {
  const documentCitations = caseInput.documents.filter(isUsableEvidenceDocument).map((doc, index) => ({
    ...
    assertionState: doc.assertionState,
  ...
  const retrievalCitations = usableRetrievalMatches(caseInput)
    .map((match, index) => ({
      ...
      assertionState: match.assertionState,
...
function assessEvidenceQuality(caseInput, citations = []) {
  ...
  const evidenceDocuments = documents.filter(isUsableEvidenceDocument);
  const retrievalMatches = usableRetrievalMatches(caseInput);
  const extractedDocuments = evidenceDocuments.filter((doc) => /parsed|retrieved|manual|text|pdf/i.test(doc.extractionStatus || '') || cleanText(doc.summary || doc.text || doc.excerpt));
  const metadataOnly = documents.filter((doc) => doc.extractionStatus === 'binary_registered');
  const substantiveDocuments = evidenceDocuments.filter((doc) => documentEvidenceText(doc).length >= 40);
  const positiveSignals = Array.from(new Set(evidenceDocuments.flatMap((doc) => Array.isArray(doc.signals) ? doc.signals.map(cleanText).filter(Boolean) : [])));
  const score = Math.min(1, Number((
    (evidenceDocuments.length ? 0.18 : 0)
  ...
  const status = score >= 0.72 ? 'strong' : score >= 0.45 ? 'usable' : documents.length || retrievalMatches.length ? 'weak' : 'missing';
  return {
    status,
    score,
...
function buildDecisionReadiness(decision) {
  return {
    status: decision.status,
    score: decision.readinessScore,
    approvalEligible: decision.approvalEligible,
    humanApprovalRequired: decision.humanApprovalRequired,
    blockingGaps: decision.blockingGaps,
    highSeverityGaps: decision.highSeverityGaps,
    mediumSeverityGaps: decision.mediumSeverityGaps,
...
function buildRetrievalAudit(caseInput) {
  ...
  const candidateMatches = Array.isArray(retrieval.matches) ? retrieval.matches : [];
  const verifiedMatches = usableRetrievalMatches(caseInput);
  return {
    mode: candidateMatches.length ? 'server_side_semantic_retrieval' : 'not_used',
    ...
    rejectedMatchCount: Math.max(0, candidateMatches.length - verifiedMatches.length),
    chunkIds: verifiedMatches.map((match) => match.chunkId).filter(Boolean).slice(0, 20),
...
function runComplianceAgent(input = {}, options = {}) {
  ...
  const verifiedRetrievalMatches = usableRetrievalMatches(caseInput);
  if (caseInput.retrievalContext?.matches?.length) {
    trace.push(makeEvent('evidence_examiner', 'semantic_retrieval_completed', {
      ...
      candidateMatchCount: caseInput.retrievalContext.matches.length,
      matchCount: verifiedRetrievalMatches.length,
      rejectedMatchCount: caseInput.retrievalContext.matches.length - verifiedRetrievalMatches.length,
      chunkCount: caseInput.retrievalContext.chunkCount,
      model: caseInput.retrievalContext.model || 'text-embedding-3-large',
      chunkIds: verifiedRetrievalMatches.map((match) => match.chunkId).filter(Boolean).slice(0, 10)
    }, root.eventId));
  }
  ...
  const policyReferenceIds = Array.from(new Set(domains.flatMap((domain) => domain.evidenceIds)));
  const citationEvidenceIds = citations.map((citation) => citation.evidenceId).filter(Boolean);
  const evidenceIds = Array.from(new Set(citationEvidenceIds));
  trace.push(makeEvent('evidence_agent', 'evidence_mapped', {
    evidenceIds,
    citationCount: citations.length,
    retrievalAugmented: verifiedRetrievalMatches.length > 0,
    gapCount: gaps.length
  }, root.eventId));

  const decision = buildDecision(gaps, evidenceQuality);
  const decisionReadiness = buildDecisionReadiness(decision);
  ...
  const outputReview = {
    ...
    checks: [
      ...
      {
        name: 'retrieval_citations',
        status: caseInput.retrievalContext?.matches?.length
          ? verifiedRetrievalMatches.length ? 'passed' : 'needs_review'
          : 'not_applicable'
      },
  ...
  return {
    ...
    domains: domains.map((domain) => ({
      ...
      applicabilityAssertions: domain.applicabilityAssertions,
...
function getReadinessInventory() {
  ...
  return {
    ...
    submissionReadiness: {
      ...
      auditTraceability: audit.enterpriseReady ? 'managed_hash_chained_audit_ready' : audit.durable ? 'durable_hash_chained_audit_needs_immutable_retention' : 'hash_chained_audit_present_local_development_only',
