...
function gatewayBaseUrl() {
  ...
  const openAiCompatibleUrl = stripTrailingSlash(process.env.OPENAI_BASE_URL || process.env.CREWAI_LLM_BASE_URL);
  return compassUrl || openAiCompatibleUrl || DEFAULT_GATEWAY_BASE_URL;
}

function gatewayToken() {
  return String(
    process.env.COMPASS_GATEWAY_TOKEN
    || process.env.PARALLAX42_GATEWAY_TOKEN
    || process.env.CREWAI_LLM_API_KEY
...
function isDirectOpenAICompatibleBase(baseUrl = gatewayBaseUrl()) {
  ...
  const explicitOpenAiBase = stripTrailingSlash(process.env.OPENAI_BASE_URL || process.env.CREWAI_LLM_BASE_URL);
  return Boolean(explicitOpenAiBase && base === explicitOpenAiBase && !/\/api$/i.test(base));
}
