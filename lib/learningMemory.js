...
const { qdrantConfig, qdrantFetch } = require('./evidenceVectorStore');
...
function learningProvider() {
  ...
  const qdrantLearningEnabled = isFeatureEnabled('qdrantLearningMemory');
  const embeddingsEnabled = isFeatureEnabled('compassEmbeddings');
  if (requested === 'qdrant') return qdrantLearningEnabled && embeddingsEnabled && qdrantConfigured ? 'qdrant' : 'local_file';
  if (!requested && qdrantLearningEnabled && embeddingsEnabled && qdrantConfigured) return 'qdrant';
  return 'local_file';
}
...
function normalizeArtifact(input = {}, options = {}) {
  ...
  const workspaceId = cleanText(actor.workspaceId || process.env.P42_WORKSPACE_ID || 'parallax42');
  const projectId = cleanText(actor.projectId || process.env.P42_PROJECT_ID || 'compliance-intelligence-agent');
...
async function searchLocalLearning(input = {}, { artifactTypes = [], limit = 5 } = {}) {
  ...
  return (memory.artifacts || [])
    .filter((artifact) => artifact.workspaceId === input.workspaceId && artifact.projectId === input.projectId)
...
async function searchQdrantLearning(input = {}, { artifactTypes = [], limit = 5 } = {}) {
  ...
  const body = await qdrantFetch(`/collections/${encodeURIComponent(qdrantConfig().collection)}/points/query`, {
    method: 'POST',
    body: JSON.stringify({
...
async function retrieveLearningArtifacts(input = {}, options = {}) {
  ...
  const scopedInput = {
    ...input,
    workspaceId: cleanText(input.workspaceId || process.env.P42_WORKSPACE_ID || 'parallax42'),
    projectId: cleanText(input.projectId || process.env.P42_PROJECT_ID || 'compliance-intelligence-agent')
  };
  if (learningProvider() === 'qdrant') {
    try {
      return await searchQdrantLearning(scopedInput, { artifactTypes, limit });
    } catch (error) {
      return [];
    }
  }
  return searchLocalLearning(scopedInput, { artifactTypes, limit });
}
...
function learningMemoryHealth() {
  ...
  return {
    ...
    localMemoryPath: provider === 'local_file' ? memoryFile() : undefined,
    features: {
      qdrantLearningMemoryEnabled: isFeatureEnabled('qdrantLearningMemory'),
