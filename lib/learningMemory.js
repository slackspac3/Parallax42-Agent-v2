...
const { qdrantConfig, qdrantFetch, vectorNamespace } = require('./evidenceVectorStore');
...
function normalizeArtifact(input = {}, options = {}) {
  ...
  const { workspaceId, projectId } = vectorNamespace({}, { actor });
...
async function retrieveLearningArtifacts(input = {}, options = {}) {
  ...
  const scopedInput = { ...input, ...vectorNamespace({}, { actor: options.actor }) };
...
async function findSimilarCases(input = {}, options = {}) {
...
async function getControlSuggestions(input = {}, options = {}) {
  const matches = await retrieveLearningArtifacts(input, {
    actor: options.actor,
