...
function fieldFromQuestion(question = '') {
  ...
  if (/owner|business unit|accountable/.test(text)) return 'business_owner';
  ...
  if (/evidence|contract|dpa|soc|iso|bcp|document|proof/.test(text)) return 'evidence';
  return '';
}
...
function inferKnownGapAnswer(text = '', previousDraft = {}) {
  ...
  const field = [...previousQuestions, ...previousAsked].map(fieldFromQuestion).find(Boolean);
...
function normalizeOwnerValue(value = '') {
  ...
  return clean.replace(/\b\w/g, (letter) => letter.toUpperCase()).replace(/\bIt\b/g, 'IT').replace(/\bAi\b/g, 'AI');
}
...
function inferBusinessOwnerAnswer(text = '', previousDraft = {}) {
  ...
  const likelyAnsweringOwner = pendingOwner && /business unit|workflow owner|case owner|accountable/i.test(previousQuestions);
  ...
  if (
    ...
    && /\b(it|legal|finance|procurement|security|compliance|risk|privacy|technology|operations|head|director|manager|owner)\b/i.test(text)
  ) {
...
function mergeDraft(existing = {}, extracted = {}) {
  ...
  return {
    ...
    knownGaps: unique([...(existing.knownGaps || []), ...(extracted.knownGaps || [])]),
...
function missingFields(draft = {}) {
  ...
  const hasEvidence = draft.evidenceSignals?.length
    || draft.documents?.some((doc) => doc.signals?.length || doc.indexStatus === 'indexed' || doc.extractionStatus === 'retrieved_chunk')
...
function questionsForDraft(draft = {}, missing = []) {
  ...
  if (missing.includes('evidence') && !knownGaps.has('evidence')) {
...
function composeReply({ intent, draft, missing, questions, run, executionBlockers = [] }) {
  ...
  if (missing.length) {
    ...
    const capturedBlock = captured.length ? `What I understood:\n${captured.map((item) => `- ${item}`).join('\n')}\n\n` : '';
    ...
    if (!executionBlockers.length) {
      ...
      return `${capturedBlock}${foundBlock}Core intake is complete. Remaining evidence/control gaps can be handled by the council if unresolved.\n\nNext question: ${next}\nWhy it matters: this keeps the deterministic decision auditable and lets the reviewer see unresolved assumptions.`;
    }
    if (questions.length) {
      return `${capturedBlock}${foundBlock}Next question: ${questions[0]}\nWhy it matters: I need this before the deterministic council can produce a review-ready decision.`;
    }
    return `${capturedBlock}${foundBlock}I recorded the missing item as a known gap. Say "run it" if you want the council to proceed with that gap visible, or add any available detail.`;
  }
  const captured = capturedContextSummary(draft);
  const capturedBlock = captured.length ? `What I understood:\n${captured.map((item) => `- ${item}`).join('\n')}\n\n` : '';
  const foundBlock = foundLines.length ? `What I found:\n${foundLines.map((line) => `- ${line}`).join('\n')}\n\n` : '';
  return `${capturedBlock}${foundBlock}I have enough context to assess this case. Say "run it" or add more evidence, and I will execute the deterministic council with human approval still required.`;
}

function filterRepeatedQuestions(questions = [], draft = {}) {
  const asked = new Set((draft.askedQuestions || []).map(normalizeQuestion));
  return questions.filter((question) => !asked.has(normalizeQuestion(question))).slice(0, 1);
}
...
function processConversation(input = {}, options = {}) {
  ...
  const questions = filterRepeatedQuestions(questionsForDraft(draft, missing), draft);
