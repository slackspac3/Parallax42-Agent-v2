...
function inferBusinessOwnerAnswer(text = '', previousDraft = {}) {
  ...
  const explicitPatterns = [
    /\b(?:business\s+unit|workflow\s+owner|case\s+owner|accountable\s+(?:team|owner|business\s+unit)|responsible\s+(?:team|owner|unit)|owner)\s*(?:is|:|=|will be|should be)?\s+(?:the\s+)?([A-Za-z][A-Za-z0-9&/ .'-]{1,80})/i,
    /\b(?:responsibility|accountability)\s+(?:sits with|belongs to|is with)\s+(?:the\s+)?([A-Za-z][A-Za-z0-9&/ .'-]{1,80})/i
  ];
...
function composeReply({ intent, draft, missing, questions, run, executionBlockers = [] }) {
  ...
  if (missing.length) {
    ...
    const capturedBlock = captured.length ? `Captured:\n${captured.map((item) => `- ${item}`).join('\n')}\n\n` : '';
    if (!executionBlockers.length) {
      return `${capturedBlock}Core intake is complete. Remaining evidence/control gaps can be handled by the council if unresolved:\n${questions.map((question) => `- ${question}`).join('\n') || '- No required follow-up before council.'}\n\nSay "run it" to execute the council, or add any detail that should strengthen the review pack.`;
    }
    return `${capturedBlock}Still needed before council:\n${questions.map((question) => `- ${question}`).join('\n')}`;
  }
  const captured = capturedContextSummary(draft);
  const capturedBlock = captured.length ? `Captured:\n${captured.map((item) => `- ${item}`).join('\n')}\n\n` : '';
  return `${capturedBlock}I have enough context to assess this case. Say "run it" or add more evidence, and I will execute the agent workflow.`;
}
...
function processConversation(input = {}, options = {}) {
  ...
  const actions = [
    ...
    { id: 'follow_up_planning', status: questions.length ? 'complete' : 'not_required', detail: questions.length ? 'Generated contextual clarification questions.' : 'No clarification required before agent execution.' },
    {
      ...
      status: draft.retrievalContext?.matches?.length ? 'complete' : draft.indexedEvidence?.chunkCount ? 'queued' : 'not_required',
      detail: draft.retrievalContext?.matches?.length
        ? `Retrieved ${draft.retrievalContext.matches.length} semantic evidence match${draft.retrievalContext.matches.length === 1 ? '' : 'es'} for council review.`
  ...
  return {
    ...
    questions,
    readyToRun: executionBlockers.length === 0,
    actions,
    run,
    nlp: {
      ...
      retrieval: {
        ...
        matches: draft.retrievalContext?.matches?.length || 0,
