...
function documentUploadQuestion(draft = {}) {
  ...
  return evidenceKnownGap
    ...
    : `Would you like to upload the ${target} now? I can analyze it first, then ask only for missing clarifications.`;
}
...
function caseShapeSummary(draft = {}) {
  ...
  if (isDocumentReviewCase(draft)) {
    ...
    return hasDocumentReviewSource(draft)
      ? `I’m treating this as ${target} review and using the attached source evidence.`
      : `I’m treating this as ${target} review. The useful next step is to upload or paste the source material.`;
  }
...
function documentAwaitingSourceReply({ draft, questions, foundLines }) {
  ...
  const sourceLine = isClauseReviewCase(draft)
    ...
    : `Upload the ${target} and I will extract the parties, scope, obligations, data terms, missing proof, and reviewer questions before asking for metadata.`;
...
function knownGapReply({ draft, questions, currentKnownGaps = [] }) {
  const currentGap = normalizeKnownGapField(currentKnownGaps[0] || '');
