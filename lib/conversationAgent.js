...
function visibleQuestionContextFromHistory(history = []) {
  ...
  const assistantTurn = turns.slice().reverse().find((turn) => (
    turn.role === 'assistant'
  ...
  if (assistantTurn) {
    return {
      question: cleanText(assistantTurn.displayedQuestion),
      id: cleanText(assistantTurn.displayedQuestionId),
      field: cleanText(assistantTurn.displayedQuestionField)
    };
  }
  const userTurn = turns.slice().reverse().find((turn) => turn.role === 'user' && turn.answeringQuestion);
  return userTurn ? {
    question: cleanText(userTurn.answeringQuestion),
    id: cleanText(userTurn.answeringQuestionId),
    field: cleanText(userTurn.answeringQuestionField)
  } : { question: '', id: '', field: '' };
}
