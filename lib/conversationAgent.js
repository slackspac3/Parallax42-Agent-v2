...
function processConversation(input = {}, options = {}) {
  ...
  const activeQuestion = cleanText(input.activeQuestion || existingDraft.activeQuestion || (Array.isArray(existingDraft.questions) ? existingDraft.questions[0] : ''));
  ...
  const extracted = extractCaseFields(message, extractionDraft);
  const answerValidation = validateActiveQuestionAnswer(message, extractionDraft, extracted);
  ...
  const draft = preserveAiDraftWhereDeterministicIsGeneric(
    extractionDraft,
    extracted,
    answerValidation.status === 'needs_clarification' && !extractedHasUsefulContext(extracted)
  ...
  if (answerValidation.status === 'needs_clarification') draft.answerValidation = answerValidation;
  ...
  let questions = answerValidation.status === 'needs_clarification'
