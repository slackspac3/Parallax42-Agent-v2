...
function readableAssessment(item = {}) {
  ...
  const assessment = cleanText(source.assessment || source.finding || source.rationale || source.summary || '');
  const risks = listBrief(source.unresolvedRisks || source.risks, 'Risks: ');
  const actions = listBrief(source.recommendedActions || source.actions, 'Actions: ');
