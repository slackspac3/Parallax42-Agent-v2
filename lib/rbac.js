...
const { authenticatePilotCookie, authenticateSessionToken } = require('./sessionService');
...
const ROUTE_POLICIES = {
  'agent:run': ['platform_admin', 'risk_admin', 'compliance_reviewer', 'legal_privacy_reviewer', 'security_reviewer', 'finance_project_reviewer', 'hse_bcm_reviewer', 'demo_user'],
  'case:approve': ['platform_admin', 'risk_admin', 'compliance_reviewer', 'business_approver', 'demo_user'],
  'learning:read': ['platform_admin', 'risk_admin', 'compliance_reviewer', 'legal_privacy_reviewer', 'security_reviewer', 'auditor', 'read_only', 'demo_user'],
  'learning:write': ['platform_admin', 'risk_admin', 'compliance_reviewer'],
  'knowledge:write': ['platform_admin', 'risk_admin'],
...
const ALWAYS_ENFORCED_POLICIES = new Set(['audit:read']);
...
function allowInsecureProductionAuth() {
  return truthy(process.env.P42_ALLOW_INSECURE_AUTH_MODE || process.env.P42_ALLOW_AUDIT_AUTH_IN_PRODUCTION);
}

function defaultAuthMode() {
  return isProductionRuntime() && !allowInsecureProductionAuth() ? 'enforced' : 'audit';
}

function authMode() {
  const fallback = defaultAuthMode();
  const raw = String(process.env.P42_AUTH_MODE || process.env.AUTH_MODE || '').trim().toLowerCase();
  const configured = ['disabled', 'audit', 'enforced'].includes(raw) ? raw : fallback;
  if (isProductionRuntime() && configured !== 'enforced' && !allowInsecureProductionAuth()) {
    return 'enforced';
  }
  return configured;
}
...
function actorWorkspaceId(subject = '', tenant = 'default') {
  if (process.env.P42_WORKSPACE_ID) return process.env.P42_WORKSPACE_ID;
  const scope = crypto.createHash('sha256').update(`${tenant}:${subject}`).digest('hex').slice(0, 20);
  return `workspace:${tenant}:${scope}`;
}
...
function validateClaims(payload = {}) {
  ...
  if (!Number.isFinite(Number(payload.exp))) throw new Error('JWT expiry is required.');
  if (Number(payload.exp) < now) throw new Error('JWT is expired.');
...
async function authenticateRequest(req) {
  ...
  if (demoToken && authorization === `Bearer ${demoToken}`) {
    return {
      ...
      workspaceId: process.env.P42_WORKSPACE_ID || 'workspace:configured-demo',
      projectId: process.env.P42_PROJECT_ID || 'parallax42-agent-v2'
    };
  }

  if (authorization.toLowerCase().startsWith('bearer ')) {
    const token = authorization.slice(7).trim();
    if (token.startsWith('p42d_')) {
      const pilotActor = await authenticatePilotCookie(req);
      if (pilotActor) return { ...pilotActor, authMode: mode };
    }
    const sessionActor = await authenticateSessionToken(token);
    if (sessionActor) return { ...sessionActor, authMode: mode };
    const payload = await verifyJwt(token);
    const subject = payload.oid || payload.sub || payload.email || 'token-subject';
    return {
      authenticated: true,
      authMode: mode,
      authSource: 'jwt',
      ...
      workspaceId: actorWorkspaceId(subject, payload.tid || 'default'),
      projectId: process.env.P42_PROJECT_ID || 'parallax42-agent-v2'
    };
  }
...
async function authorizeRequest(req, policyId) {
  const mode = authMode();
  const policyRoles = ROUTE_POLICIES[policyId] || [];
  const enforced = mode === 'enforced' || ALWAYS_ENFORCED_POLICIES.has(policyId);
  try {
    const actor = await authenticateRequest(req);
    ...
    if (!actor.authenticated && enforced) {
      return {
        ok: false,
        statusCode: 401,
        actor,
    ...
    if (!enforced) {
      return {
        ...
        actor: {
          authenticated: false,
          authMode: mode,
          authSource: 'invalid_bearer_ignored',
          authWarning: error instanceof Error ? error.message : 'Invalid bearer token ignored in audit mode.',
          id: 'anonymous',
          username: 'browser_operator',
          roles: ['demo_user']
        },
        policy: { id: policyId, requiredRoles: policyRoles, mode }
      };
    }
    const actor = {
      authenticated: false,
...
function authHealth() {
  ...
  return {
    ...
    productionDefaultEnforced: isProductionRuntime() && !allowInsecureProductionAuth(),
    jwt: {
