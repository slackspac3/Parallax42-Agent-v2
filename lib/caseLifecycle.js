...
const { evidenceAssertionState, isUsableEvidenceDocument, normaliseEvidenceDocument } = require('./evidenceAssertions');
...
function canonicalDemoFixture(document = {}) {
  const fileName = cleanText(document.fileName || document.filename || document.metadata?.fileName);
  const evidenceId = cleanText(document.evidenceId || document.metadata?.evidenceId);
  const fixture = getFixtureDocumentByFilename(fileName);
  if (!fixture || evidenceId !== `FIXTURE-${fixture.filename.split('_', 1)[0]}`) return null;
  return fixtureDocumentSummary(fixture.filename).evidence;
}

function governedSessionDocument(actor = {}, document = {}) {
  const fixture = actor.sessionType === 'demo' ? canonicalDemoFixture(document) : null;
  if (fixture) return { ...fixture, provenance: 'demo_fixture', assertionState: 'parsed' };
  const unverified = normaliseEvidenceDocument({
    ...document,
  ...
  return {
    ...unverified,
    provenance: 'chat_message',
    assertionState: evidenceAssertionState({ ...unverified, provenance: 'chat_message' })
  };
}

function durableDraftFields(draft = {}, actor = {}) {
  const documents = Array.isArray(draft.documents) ? draft.documents.slice(0, 12).map((document = {}) => governedSessionDocument(actor, document)).map((document = {}) => ({
    ...
    provenance: cleanText(document.provenance),
    assertionState: cleanText(document.assertionState),
...
function isClientGeneratedCaseId(value = '') {
  return /^case_[a-z0-9][a-z0-9_-]{7,95}$/i.test(cleanText(value));
}

function usableEvidenceIds(documents = []) {
  return documents
    .filter(isUsableEvidenceDocument)
    .map((document) => cleanText(document.evidenceId))
    .filter(Boolean);
}
...
async function saveCaseDraft(actor, draft = {}, { expectedVersion } = {}) {
  ...
  const fields = durableDraftFields(draft, actor);
  ...
  if (!current) {
    const createsVersionZeroCase = requestedCaseId
      && expectedVersion !== undefined
      && expectedVersion !== null
      && Number(expectedVersion) === 0
      && isClientGeneratedCaseId(requestedCaseId);
    if (requestedCaseId && !createsVersionZeroCase && (actor.sessionType || expectedVersion !== undefined)) {
      const error = new Error('Case was not found in this workspace.');
      error.statusCode = 404;
      error.code = 'case_not_found';
      throw error;
    }
    if (requestedCaseId && !createsVersionZeroCase) caseId = crypto.randomUUID();
    return publicCase(await putRecord({
      ...
      data: {
        ...
        ownerId: actor.id,
        evidenceIds: usableEvidenceIds(fields.documents),
  ...
  const updated = await updateRecord('case', caseId, (record) => {
    ...
    const priorRunEvidenceIds = Array.isArray(record.data.lastRun?.evidenceIds) ? record.data.lastRun.evidenceIds : [];
    return {
      data: {
        ...record.data,
        ...fields,
        state: mutableState || invalidatesDecision ? nextState : record.data.state,
        evidenceIds: Array.from(new Set([...priorRunEvidenceIds, ...usableEvidenceIds(fields.documents)])),
