...
function shouldAcceptBusinessUnitUpdate({ draft = {}, assessment = {}, message = '', value = '' } = {}) {
  const existing = cleanText(draft.businessUnit);
  const next = cleanText(value);
  if (!next) return false;
  if (existing && existing.toLowerCase() === next.toLowerCase()) return true;
  if (existing) {
    const grounded = businessUnitAppearsInText(next, message);
    const explicitUpdate = /\b(?:change|update|replace|correct|instead|now|owner|owned\s+by|business\s+unit)\b/i.test(message);
    return grounded && (
      assessment.intent === 'owner_answer'
      || activeFieldForDraft(draft) === 'business_owner'
      || explicitUpdate
    );
  }
...
function shouldAcceptGeographyUpdate({ draft = {}, assessment = {}, message = '', value = '' } = {}) {
  ...
  if (existing && existing.toLowerCase() === next.toLowerCase()) return true;
  if (existing) {
    const grounded = geographyAppearsInText(next, message);
    const explicitUpdate = /\b(?:change|update|replace|correct|instead|now|geograph|location|country|region|jurisdiction)\w*\b/i.test(message);
    return grounded && (
      assessment.intent === 'geography_answer'
      || activeFieldForDraft(draft) === 'geography'
      || explicitUpdate
    );
  }
...
function mergeAssessmentIntoDraft(draft = {}, assessment = {}, options = {}) {
  ...
  const preserveCaseFacts = cleanText(options.eventType).toLowerCase() === 'evidence_uploaded';
