...
const DEFAULT_CONVERSATION_LLM_MAX_TOKENS = 1500;
const DEFAULT_CONVERSATION_LLM_RETRY_MAX_TOKENS = 1100;
...
function assessmentContext(message = '', draft = {}, options = {}) {
  ...
  return {
    ...
    requiredSchema: {
      ...
      naturalResponse: 'actual assistant message to show the user; acknowledge the latest message, reference named context, ask at most one follow-up question',
      caseUpdate: {
    ...
    return [
      {
        ...
        content: [
          ...
          'You assess and respond to a compliance intake chat turn.',
          'Include naturalResponse as the exact concise assistant message to show the user.',
          'naturalResponse must acknowledge the latest message, reference named prior context when available, ask at most one follow-up question, and sound like a knowledgeable compliance advisor.',
          ...
          'If the user\'s last message is a direct answer to your last question, naturalResponse must confirm the answer and move to the next topic. Never repeat or rephrase the same question.',
  ...
  return [
    {
      ...
      content: [
        ...
        'You are not classification-only: produce both structured fields and naturalResponse.',
        'naturalResponse is the exact assistant message that will be sent to the user.',
        'naturalResponse should be concise, natural, and business-facing. It must acknowledge what you understood from the latest user message, reference prior context by name when available such as supplier, document, geography, systems, owner, or uploaded file, ask at most one follow-up question, and explain the reason only if useful.',
        'If you have already acknowledged a piece of information in a prior turn, do not re-acknowledge it; move forward.',
        'Never ask for something the user has already provided, even if it is not yet in currentDraft. Track what you have already understood from the conversation history.',
        'If the user\'s last message is a direct answer to your last question, naturalResponse must confirm the answer and move to the next topic. Never repeat or rephrase the same question.',
