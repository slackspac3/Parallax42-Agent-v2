...
function qdrantConfigured() {
  return vectorConfig().qdrantConfigured;
}

function compassGatewayConfigured() {
  return gatewayConfig().tokenConfigured;
}

function compassGatewayBaseUrlPresent() {
  return gatewayConfig().baseUrlPresent;
}

function parserRelayBaseUrlPresent() {
  return Boolean(parserRelayConfig().backendUrl);
}
...
function featureRequirements(id) {
  ...
  const gatewayConfigured = gateway.tokenConfigured;
  const demoEmbeddingsConfigured = parseBoolean(process.env.P42_DEMO_EMBEDDINGS) === true;
  const qdrantReady = vector.qdrantConfigured;
  const base = {
    configured: true,
    active: true,
    unmet: []
  ...
  if (id === 'compassLlmCalls') {
    return {
      ...
      unmet: gatewayConfigured ? [] : ['COMPASS_GATEWAY_TOKEN, PARALLAX42_GATEWAY_TOKEN, CREWAI_LLM_API_KEY, or OPENAI_API_KEY']
    };
  }
  ...
  if (id === 'qdrantRag') {
    ...
    const embeddingsReady = demoEmbeddingsConfigured || (gatewayConfigured && embeddings.enabled);
    const unmet = [];
    if (!qdrantReady) unmet.push('QDRANT_URL or P42_VECTOR_DB_URL');
    if (!embeddingsReady) unmet.push('P42_DEMO_EMBEDDINGS=true or configured Compass embeddings');
    return {
      configured: qdrantReady && embeddingsReady,
      active: qdrantReady && embeddingsReady,
      unmet,
      note: demoEmbeddingsConfigured
        ? 'Qdrant uses deterministic demo hash embeddings; no Compass semantic-embedding claim is made.'
        : 'Qdrant uses the configured Compass-compatible embedding boundary.'
    };
  }
  if (id === 'qdrantLearningMemory') {
    ...
    return {
      ...
      unmet,
      note: demoEmbeddingsConfigured
        ? 'Learning memory uses deterministic demo hash embeddings.'
        : 'Learning memory uses the configured Compass-compatible embedding boundary.'
    };
  }
  ...
  if (id === 'liveAdvisorySpecialists') {
    ...
    if (!gatewayConfigured) unmet.push('COMPASS_GATEWAY_TOKEN, PARALLAX42_GATEWAY_TOKEN, CREWAI_LLM_API_KEY, or OPENAI_API_KEY');
    return {
      configured: gatewayConfigured,
...
function buildFeatureStatus() {
  ...
  return {
    ...
    gateway: {
      ...
      llmModel: gateway.llmModel,
      embeddingsModel: gateway.embeddingsModel
    },
    qdrant: {
      configured: vector.qdrantConfigured,
      provider: vector.provider,
      collection: vector.collection
    },
    parserRelay: {
      configured: parser.configured,
      baseUrlPresent: Boolean(parser.backendUrl)
    },
    crewai: {
