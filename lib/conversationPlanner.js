...
function buildConversationPlan(body = {}) {
  ...
  return {
    ...
    source: assessment?.used ? 'compass_gpt5_1_planner' : smartIntakeUnavailable ? 'compass_required_unavailable' : 'deterministic_planner_fallback',
    smartIntakeUnavailable,
    requiresCompass: smartIntakeUnavailable,
    userMessage: smartIntakeUnavailable ? assessment?.userMessage || SMART_INTAKE_UNAVAILABLE_MESSAGE : '',
    ...
    nextQuestion: smartIntakeUnavailable ? assessment?.userMessage || SMART_INTAKE_UNAVAILABLE_MESSAGE : cleanText(assessment?.nextBestQuestion),
    reason: smartIntakeUnavailable ? assessment?.userMessage || SMART_INTAKE_UNAVAILABLE_MESSAGE : cleanText(assessment?.reason),
    assistantSummary: smartIntakeUnavailable ? assessment?.userMessage || SMART_INTAKE_UNAVAILABLE_MESSAGE : cleanText(assessment?.assistantSummary),
