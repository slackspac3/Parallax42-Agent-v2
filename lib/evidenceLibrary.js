...
function sourceBlob(input = {}) {
  ...
  return [
    ...
    input.businessUnit,
    input.geography,
    input.exportOriginJurisdiction,
    input.exportEndUse,
    input.serviceDescription,
    input.supplierName,
    input.reviewFocus,
    JSON.stringify(input.aiUsageScope || {}),
    ...(Array.isArray(input.dataCategories) ? input.dataCategories : []),
    ...(Array.isArray(input.riskSignals) ? input.riskSignals : []),
    ...(Array.isArray(input.evidenceSignals) ? input.evidenceSignals : []),
    ...(Array.isArray(input.knownGaps) ? input.knownGaps : []),
    ...(Array.isArray(input.sanctionsSensitiveGeographies) ? input.sanctionsSensitiveGeographies : []),
