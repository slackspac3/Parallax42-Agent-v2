...
function vectorNamespace(payload = {}, options = {}) {
  ...
  const baseWorkspaceId = configuredText(process.env.P42_WORKSPACE_ID) || (trustClientNamespace ? configuredText(payload.workspaceId) : '') || 'parallax42';
  const projectId = configuredText(process.env.P42_PROJECT_ID) || (trustClientNamespace ? configuredText(payload.projectId) : '') || 'compliance-intelligence-agent';
  const scope = actorScopeId(options.actor || payload.actor);
  return {
    workspaceId: scope ? `${baseWorkspaceId}:actor:${scope}` : baseWorkspaceId,
    projectId,
    actorScoped: Boolean(scope)
  };
}
...
async function searchQdrantPoints(payload = {}, { includeCase = true } = {}) {
  ...
  const matches = (Array.isArray(body.result) ? body.result : []).map((point) => safeMatchForClient({
    ...(point.payload || {}),
...
function evidenceVectorStoreHealth() {
  ...
  const embeddingsFeatureEnabled = isFeatureEnabled('compassEmbeddings');
  return {
    provider,
    ...
    features: {
      ...
      compassEmbeddingsEnabled: embeddingsFeatureEnabled
    },
    browserEmbeddingsRetained: false,
