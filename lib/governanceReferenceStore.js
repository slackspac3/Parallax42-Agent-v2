...
const { qdrantConfig, qdrantFetch, stableUuid, vectorNamespace } = require('./evidenceVectorStore');
...
const PUBLIC_REFERENCE_WORKSPACE_ID = 'parallax42:public-governance';
const PUBLIC_REFERENCE_PROJECT_ID = 'compliance-intelligence-agent:public-reference';
...
function referenceNamespace(payload = {}, options = {}) {
  // Offline import scripts may opt in; request handlers must pass an authenticated actor.
  if (options.trustedNamespace === true) {
    if (payload.publicSafe !== false) {
      return {
        workspaceId: PUBLIC_REFERENCE_WORKSPACE_ID,
        projectId: PUBLIC_REFERENCE_PROJECT_ID
      };
    }
    return {
      workspaceId: cleanText(payload.workspaceId || process.env.P42_WORKSPACE_ID) || 'parallax42',
      projectId: cleanText(payload.projectId || process.env.P42_PROJECT_ID) || 'compliance-intelligence-agent'
    };
  }
  return vectorNamespace({}, { actor: options.actor });
}
...
function chunkGovernanceReference(payload = {}) {
  ...
  sections.flatMap((section) => splitLongSection(section)).forEach((section, index) => {
    ...
    chunks.push({
      ...
      corpusType: cleanText(payload.corpusType),
      lane: cleanText(payload.lane),
      ...
      sharedPublicCorpus: payload.sharedPublicCorpus === true,
...
async function indexGovernanceReference(payload = {}, options = {}) {
  const { workspaceId, projectId } = referenceNamespace(payload, options);
  const sourceId = cleanText(payload.sourceId) || DEFAULT_SOURCE_ID;
  const provider = referenceProvider();
  const sharedPublicCorpus = options.trustedNamespace === true && payload.publicSafe !== false;
  const chunks = chunkGovernanceReference({ ...payload, sharedPublicCorpus });
...
async function searchQdrantReferences(payload = {}) {
  ...
  if (payload.publicOnly) must.push({ key: 'sharedPublicCorpus', match: { value: true } });
  const { collection } = qdrantConfig();
  ...
  return {
    ...
    references: points.map((point) => safeReferenceForClient({
      ...(point.payload || {}),
...
async function searchGovernanceReferences(payload = {}, options = {}) {
  const query = cleanText(payload.query || '');
  const { workspaceId, projectId } = referenceNamespace(payload, options);
  const includePublicCorpus = options.trustedNamespace !== true
    && (workspaceId !== PUBLIC_REFERENCE_WORKSPACE_ID || projectId !== PUBLIC_REFERENCE_PROJECT_ID);
  ...
  if (provider === 'qdrant') {
    const searches = [searchQdrantReferences({ ...payload, query, workspaceId, projectId, sourceId })];
    if (includePublicCorpus) {
      searches.push(searchQdrantReferences({
        ...payload,
        query,
        workspaceId: PUBLIC_REFERENCE_WORKSPACE_ID,
        projectId: PUBLIC_REFERENCE_PROJECT_ID,
    ...
    results.flatMap((item) => item.references || []).forEach((reference) => {
      const key = reference.referenceId || reference.chunkId;
      if (!referenceMap.has(key) || Number(reference.score || 0) > Number(referenceMap.get(key).score || 0)) {
    ...
    return {
      ...
      context: { workspaceId, projectId, sourceId, purpose: payload.purpose || 'governance_reference_search', publicCorpusIncluded: includePublicCorpus },
      index: {
        provider: 'qdrant',
        storage: 'server_side_qdrant_governance_reference_store',
  ...
  const actorChunks = localReferenceChunks({ workspaceId, projectId, sourceId });
  const publicChunks = includePublicCorpus
    ? localReferenceChunks({ workspaceId: PUBLIC_REFERENCE_WORKSPACE_ID, projectId: PUBLIC_REFERENCE_PROJECT_ID, sourceId })
      .filter((chunk) => chunk.sharedPublicCorpus === true && chunk.publicSafe !== false)
    : [];
  const chunks = [...actorChunks, ...publicChunks];
...
module.exports = {
  PUBLIC_REFERENCE_PROJECT_ID,
  PUBLIC_REFERENCE_WORKSPACE_ID,
