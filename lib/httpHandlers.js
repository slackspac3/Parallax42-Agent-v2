...
async function recoverFailedCouncil(context, error) {
  ...
  try {
    return await failCouncil(context.actor, context.caseId, error);
  } catch {
...
function recoveredCaseBody(recoveredCase) {
  if (!recoveredCase || typeof recoveredCase !== 'object') return {};
  const snapshot = { ...recoveredCase };
  const version = Number(snapshot.version || 0);
  delete snapshot.lastRun;
  delete snapshot.version;
  return {
    caseVersion: version,
    recoveredCase: { ...snapshot, version },
    caseDraft: { ...snapshot, caseVersion: version }
  };
}

function authoritativeDraft(storedCase, fallback = {}) {
  if (!storedCase || typeof storedCase !== 'object') return fallback;
  const snapshot = { ...storedCase };
  const version = Number(snapshot.version || 0);
  delete snapshot.lastRun;
  delete snapshot.version;
  return { ...fallback, ...snapshot, caseVersion: version };
}

async function hydrateRunRequestFromStoredCase(body = {}, actor = {}, loadCase = getCase) {
  const draft = body.caseDraft && typeof body.caseDraft === 'object' ? body.caseDraft : {};
  const runRequest = Boolean(body.forceRun || String(body.eventType || '').toLowerCase() === 'run_request');
  const caseId = String(draft.caseId || body.caseId || '').trim();
  if (!runRequest || !actor?.authenticated || !actor.workspaceId || !caseId) return body;
  const storedCase = await loadCase(actor, caseId);
  if (!storedCase) return body;
  return {
    ...body,
    caseDraft: authoritativeDraft(storedCase, draft)
  };
}
...
async function handleStandardRun({ req, body = {}, startedAt = Date.now() } = {}) {
  ...
  try {
    ...
    if (storedDraft) normalized.caseDraft = authoritativeDraft(storedDraft, normalized.caseDraft);
    ...
    const recoveredCase = persistedCase || await recoverFailedCouncil(activeCouncil, error);
    return {
      status: error.statusCode || 500,
      body: {
        ...buildEvaluatorError({ normalized, error, startedAt }),
        error: error.code || 'run_failed',
        ...recoveredCaseBody(recoveredCase)
      }
    };
  }
}
...
async function handleAgentRun({ req, body = {} } = {}) {
  ...
  try {
    ...
    if (storedDraft) agentInput = authoritativeDraft(storedDraft, agentInput);
...
async function handleConversation({ req, body = {}, dependencies = {} } = {}) {
  ...
  try {
    ...
    const expectedCaseVersion = body.caseVersion ?? body.caseDraft?.caseVersion;
    await consumeQuota(authorized.auth.actor, 'chatTurns');
    const hydratedBody = await hydrateRunRequestFromStoredCase(
      body,
      authorized.auth.actor,
      dependencies.getCase || getCase
    );
    const assessedBody = await planTurn(hydratedBody, { actor: authorized.auth.actor });
    ...
    const storedDraft = await saveCaseDraft(authorized.auth.actor, result.caseDraft, {
      expectedVersion: expectedCaseVersion ?? result.caseDraft?.caseVersion
    });
    if (storedDraft) {
      result.caseDraft = authoritativeDraft(storedDraft, result.caseDraft);
    }
    if (result.shouldRun) {
      ...
      activeCouncil = null;
      if (completedCase) {
        const completedCaseSnapshot = { ...completedCase };
        const { version } = completedCaseSnapshot;
        delete completedCaseSnapshot.lastRun;
        delete completedCaseSnapshot.version;
        result.caseVersion = version;
        result.run.caseVersion = version;
        result.completedCase = { ...completedCaseSnapshot, version };
        result.caseDraft = {
          ...result.caseDraft,
          ...completedCaseSnapshot,
          caseVersion: version
        };
      }
...
async function handleEvidenceIndex({ req, body = {} } = {}) {
  try {
    ...
    await appendAuditRecord(evidenceIndexAuditPayload({ auth: authorized.auth, body: governedBody, result }));
    return { status: 200, body: result };
  } catch (error) {
    return {
...
async function handleEvidenceSearch({ req, body = {} } = {}) {
  try {
    ...
    await appendAuditRecord(evidenceSearchAuditPayload({ auth: authorized.auth, body, result }));
    return { status: 200, body: result };
  } catch (error) {
    return {
      status: error.statusCode || error.status || 502,
