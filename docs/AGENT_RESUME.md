...
- Obligation mapping emits named evidence IDs with assertion state/provenance; questions, mentions, placeholders, and policy references cannot satisfy controls.
...
- Uses active Node specialists for advisory analysis while Python CrewAI remains optional/inactive. Node is the sole final authority, and Python preserves its policy fields unchanged.
